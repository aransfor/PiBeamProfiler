...
class Gaussian2D(object):
	...
	self._pixel_res = 100
	self._a_pixel_factor = self._a_pixel_len / self._pixel_res	#Factors for rescaling the Gauss widths according
	self._b_pixel_factor = self._b_pixel_len / self._pixel_res	#to the pixel size and picture resolution
	...
	self._w_a_pix = self._a_pixel_factor * self._w_a		#Scales the Gauss widths in terms of pixels
	self._w_b_pix = self._b_pixel_factor * self._w_b 
	self._w_a_len = self._a_pixel_factor * self._pixel_size * self._w_a * 1000	#Scales the Gauss widths in terms of mm
	self._w_b_len = self._b_pixel_factor * self._pixel_size * self._w_b * 1000
